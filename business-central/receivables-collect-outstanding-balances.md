...
For example, if you write **You owe %9 %7 due on %2.** , then the resulting reminder will contain the following text: **You owe INR 1,200.50 due on 02-02-2014.** .
...
7. Select the **Only Entries with Overdue Amounts** check box if you want the reminders to contain only overdue open entries. Only invoices and payments will be shown as these are the entries for which your customers' payments may be overdue.
    ...
    > [!Important]
    > Open entries that are on hold will be inserted, irrespective of the settings in the **Only Entries with Overdue Amounts** check box.
