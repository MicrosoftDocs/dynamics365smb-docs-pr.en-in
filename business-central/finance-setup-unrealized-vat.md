...
If you are using cash-based accounting methods, you can set up [!INCLUDE[d365fin](includes/d365fin_md.md)] to handle unrealised VAT.
...
5. In the **Unrealised VAT Type** field, choose an option to specify how to allocate payments to the invoice amount (excluding VAT) and the VAT amount itself, and how to transfer VAT amounts from the unrealised VAT account to the realised account. The following table describes the options.
...
6. In the **Sales VAT Unreal. Account** field, choose the account for unrealised sales VAT.
